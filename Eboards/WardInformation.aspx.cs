...
namespace EBoards
{
    public partial class WardInformation : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                Response.Cookies[Session["SynapseUser_UserID"].ToString().ToLower() + "_SynapseBedBoards_ReturnURL"].Value = "WardInformation.aspx";
        ...
        {
            string sql = "select * from entitystorematerialised.local_eboards_wardinformation where wardcode = @wardcode;";
            ...
            if(dt.Rows.Count == 0)
            {
                string sqlInsert = "INSERT INTO entitystore.local_eboards_wardinformation (eboards_wardinformation_id, wardcode) values (@wardcode, @wardcode);";
                var paramListInsert = new List<KeyValuePair<string, string>>() {
                new KeyValuePair<string, string>("wardcode", this.ddlSelectedWard.SelectedValue)
                };

                try
                {
                     DataServices.ExcecuteNonQueryFromSQL(sqlInsert, paramListInsert);                    
                }
                catch (Exception ex)
                {
                }
            ...
            try
            {
                wardinformation = dt.Rows[0]["wardinformation"].ToString();
            }
            catch
            {                
            }

            this.txtWardInformation.Text = wardinformation;
        ...
        {
            //string sqlUpdate = "Update entitystore.local_eboards_wardinformation SET wardinformation = @wardinformation WHERE wardcode = @wardcode;";
            //var paramListUpdate = new List<KeyValuePair<string, string>>() {
            //    new KeyValuePair<string, string>("wardinformation", this.txtWardInformation.Text),
            //    new KeyValuePair<string, string>("wardcode", this.ddlSelectedWard.SelectedValue)
            //    };
            ...
            string WardInformation = this.txtWardInformation.Text;

            string sqlInsert = "DELETE FROM  entitystorematerialised.local_eboards_wardinformation WHERE wardcode = @wardcode;  INSERT INTO entitystorematerialised.local_eboards_wardinformation (eboards_wardinformation_id, wardcode, wardinformation, _createdby) values (@wardcode, @wardcode, @wardinformation, @_createdby);";
            var paramListInsert = new List<KeyValuePair<string, string>>() {
                new KeyValuePair<string, string>("wardinformation", WardInformation),
                new KeyValuePair<string, string>("wardcode", this.ddlSelectedWard.SelectedValue),
                new KeyValuePair<string, string>("_createdby", this.lblUserFullName.Text)
                };
