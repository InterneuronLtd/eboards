...
namespace EBoards
{
    public partial class locatorcallback : System.Web.UI.Page
    {
        ...
        protected void btnPostback_Click(object sender, EventArgs e)
        {
            ...
            if (token != string.Empty)
            {
                ...
                {
                    Response.Redirect("LocatorLogOut.aspx");
                }


                if (((JwtSecurityToken)jwttoken).Claims.Count() > 0)
                {
                    ...
                    if (userFullName != null)
                        if (userFullName.Value.Split(' ').Length > 1)
                        {
                            ...
                            Session["SynapseUser_FullName"] = firstName + " " + lastName;
                        }
                        else
                        { Session["SynapseUser_FullName"] = userFullName.Value; }
                    else
                    {
                        recordFailedLoginAttempt("-1", IPAddress, "unknown");
                        Response.Redirect("LocatorSessionExpired.aspx");
                    ...
                    string returnURL = "LocatorDefault.aspx";
                    try
                    {
                        returnURL = Request.Cookies[Session["SynapseUser_UserID"].ToString().ToLower() + "_Locator_SynapseBedBoards_ReturnURL"].Value.ToString(); ;
                    }
                    catch { }

                    recordSuccessfulLoginAttempt("0", winAccNameClaim.Value, IPAddress, "Active Directory");
