...
namespace EBoards
{
    public partial class LocatorBoard : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("LocatorSessionExpired.aspx");
                }
                ...
                if(id == "5b3a8c1f-629d-4893-a9d9-ab49d20d53f2")
                {
                    Response.Redirect("LocatorBoard.aspx?id=4908f40e-ab50-4cd9-ad83-b6fbf4bb634d");
                }
                if (string.IsNullOrWhiteSpace(id))
                {
                    Response.Redirect("Default.aspx");
                }
                Response.Cookies[Session["SynapseUser_UserID"].ToString().ToLower() + "_Locator_SynapseBedBoards_ReturnURL"].Value = "LocatorBoard.aspx?id=" + id;


                this.lblUserFullName.Text = Session["SynapseUser_FullName"].ToString();

                this.lblNavIMAPage.Text = GetListDetailsByLocatorBoardID(id);
                ...
                try
                {
                    previouslySelectedWard = Request.Cookies[Session["SynapseUser_UserID"].ToString().ToLower() + "_Locator_SynapseBedBoards_SelectedWard"].Value;
                }
                catch { }
        ...
        private string GetListDetailsByLocatorBoardID(string locatorboard_id)
        {
            ...
            try
            {
                listname = dt.Rows[0]["locatorboardname"].ToString();
            }
            catch
            {
                return "";
        ...
        {
            //GetNavLabelCounts();
            Response.Cookies[Session["SynapseUser_UserID"].ToString().ToLower() + "_Locator_SynapseBedBoards_SelectedWard"].Value = this.ddlSelectedWard.SelectedValue;
        }


        protected void lbtnNavCurrentPatients_Click(object sender, EventArgs e)
        {
            Response.Cookies["_LocatorIMAPage"].Value = "Current Patients";
            Response.Redirect("_LocatorIMA.aspx?id=" + this.hdn_currentpatients_locatorboard_id.Value);
        }

        protected void lbtnNavWaitingArea_Click(object sender, EventArgs e)
        {
            Response.Cookies["_LocatorIMAPage"].Value = "Waiting Area";
            Response.Redirect("_LocatorIMA.aspx?id=" + this.hdn_waitingarea_locatorboard_id.Value);
        }

        protected void lbtnNavTCIs_Click(object sender, EventArgs e)
        {
            Response.Cookies["_LocatorIMAPage"].Value = "TCIs";
            Response.Redirect("_LocatorIMA.aspx?id=" + this.hdn_tcis_locatorboard_id.Value);
        }

        protected void lbtnNavRecentPatients_Click(object sender, EventArgs e)
        {
            Response.Cookies["_LocatorIMAPage"].Value = "Recent Patients";
            Response.Redirect("_LocatorIMA.aspx?id=" + this.hdn_tcis_locatorboard_id.Value);
        }
